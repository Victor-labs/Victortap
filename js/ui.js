...
function getTapBonus(){
  ...
  try{if(G.en&&G.en.bc&&G.en.bc.active)b+=2;}catch(e){}
  try{if(G.bs&&G.bs.sr&&G.bs.sr.active&&now<G.bs.sr.endAt)b+=5;}catch(e){}
  try{if(G.en&&G.en.xm&&G.en.xm.active&&now<G.en.xm.endAt)b+=0.1;}catch(e){}
  return b;
}
...
function renderAll(){
  try{
  if(!G) return;
  if(!G.en) G.en={bc:{unlocked:false,active:false},ce:{unlocked:false,active:false},xm:{unlocked:false,active:false,endAt:0,cdEnd:0}};
  if(!G.bs) G.bs={sr:{owned:false,active:false,endAt:0},dr:{owned:false,active:false,endAt:0,tick:0},vs:{owned:false,active:false,endAt:0},gr:{owned:false,active:false,endAt:0}};
  ...
  if(G.en&&G.en.bc&&G.en.bc.active)lines.push('<span style="color:var(--green);">🛡️ Buff Control — ACTIVE (+2/tap)</span>');
  if(G.en&&G.en.ce&&G.en.ce.active)lines.push('<span style="color:var(--teal);">🌌 Celestial — ACTIVE (mine=100💎)</span>');
  if(G.en&&G.en.xm&&G.en.xm.active&&now<G.en.xm.endAt)lines.push('<span style="color:var(--purple);">🧲 XP Magnet — '+fmT(Math.ceil((G.en.xm.endAt-now)/1000))+' left</span>');
  if(typeof BOOSTS!=='undefined') BOOSTS.forEach(function(b){var bs=G.bs[b.id];if(bs&&bs.active&&now<bs.endAt)lines.push('<span style="color:var(--orange);">'+b.ico+' '+b.name+' — '+fmT(Math.ceil((bs.endAt-now)/1000))+' left</span>');});
...
function startTick(){
  setInterval(function(){
    ...
    if(typeof BOOSTS!=='undefined') BOOSTS.forEach(function(b){var bs=G.bs&&G.bs[b.id];if(bs&&bs.active&&now>=bs.endAt){bs.active=false;ch=true;toast('⏰ '+b.name+' expired!','#ff7b35');}});
    if(G.en&&G.en.xm&&G.en.xm.active&&now>=G.en.xm.endAt){G.en.xm.active=false;G.en.xm.cdEnd=now+50*3600000;ch=true;toast('🧲 XP Magnet expired! 50hr cooldown.','#b06aff');}
    if(ch){sv();renderAll();}
    try{renderBoosts();}catch(e){} try{renderEnigma();}catch(e){} try{applyProfilePic();}catch(e){} try{injectStoreBtn();}catch(e){}
    if(typeof tickExplore==='function') try{tickExplore();}catch(e){}
  },1000);
}
